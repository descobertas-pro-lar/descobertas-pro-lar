...
const PRODUCT_FAMILY_PATTERNS = [
  ['food-containers', /\b(pote|potes|porta mantimento|porta mantimentos|marmita|marmitas)\b|\b(bowl|bowls|tigela|tigelas)\b.*\b(tampa|hermetic[oa]s?)\b/],
...
function productFamily(product) {
  ...
  return match ? match[0] : `other:${title.split(' ').slice(0, 4).join('-')}`;
}
  ...
  while (selected.length < limit) {
    ...
    for (let index = 0; index < candidateBatches.length; index += 1) {
      ...
      while (cursors[index] < batch.length) {
        ...
        if (!usedIds.has(id) && !usedTitles.has(title) && !usedFamilies.has(family)) {
          usedIds.add(id);
